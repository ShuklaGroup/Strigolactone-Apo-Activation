...
def run_all(msm, initial_state, nsteps, topfile, traj_list, save_name="kmc_traj"):
    ...
    get_structure_traj(state_traj, msm, topfile, sorted(glob.glob("./%s/*"%traj_list)), split=True)
    #structure_traj = get_structure_traj(state_traj, msm, topfile, sorted(glob.glob("./%s/*"%traj_list)), split=True)
